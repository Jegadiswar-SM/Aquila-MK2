...
//   2. Replaced raw `rst_n` with `rst_n_sync` in all submodules and local registers.
//   3. Changed all local sequential blocks (always blocks) to use async reset
//      with `always @(posedge clk or negedge rst_n_sync)`.
...
module rls_dnn_top (
    ...
    poly_decim u_poly (
        .clk               (clk),
        .rst_n             (rst_n_sync),
        .sample_enable_in  (sample_enable),
    ...
    always @(posedge clk or negedge rst_n_sync) begin
        if (!rst_n_sync || plwd_pipeline_srst) begin
            for (i_dly = 0; i_dly < 6; i_dly = i_dly + 1)
                d_dly[i_dly] <= 16'sd0;
        end else if (sample_consumed_cbpf) begin
            d_dly[0] <= d_in;
            for (i_dly = 1; i_dly < 6; i_dly = i_dly + 1)
                d_dly[i_dly] <= d_dly[i_dly-1];
        end
    end
    ...
    reg decim_q;
    always @(posedge clk or negedge rst_n_sync) begin
        if (!rst_n_sync || plwd_pipeline_srst)
            decim_q <= 1'b1;
        else if (sample_consumed_cbpf)
            decim_q <= poly_rls_en;
    end
    ...
    cbpf_2sos u_cbpf (
        .clk             (clk),
        .rst_n           (rst_n_sync && !plwd_pipeline_srst),
        .x_in            (x_in),
        .x_valid         (sample_consumed_cbpf),
        .s_axi_awaddr    (s_axi_awaddr[3:0]),
        .s_axi_awvalid   (cbpf_aw_sel && s_axi_awvalid),
        .s_axi_wdata     (s_axi_wdata),
        .s_axi_wvalid    (cbpf_aw_sel && s_axi_wvalid),
    ...
    aad u_aad (
        ...
        .s_axi_awaddr    (s_axi_awaddr[3:0]),
        .s_axi_awvalid   (aad_aw_sel && s_axi_awvalid),
        .s_axi_wdata     (s_axi_wdata),
        .s_axi_wvalid    (aad_aw_sel && s_axi_wvalid),
    ...
    rcim u_rcim (
        ...
        .s_axi_awvalid     (rcim_aw_sel && s_axi_awvalid),
        .s_axi_wdata       (s_axi_wdata),
        .s_axi_wvalid      (rcim_aw_sel && s_axi_wvalid),
    ...
    affhc u_affhc (
        ...
        .e_valid          (rls_valid_out),
        .s_axi_awaddr     (s_axi_awaddr[4:0]),
        .s_axi_awvalid    (affhc_aw_sel && s_axi_awvalid),
        .s_axi_wdata      (s_axi_wdata),
        .s_axi_wvalid     (affhc_aw_sel && s_axi_wvalid),
    ...
    plwd u_plwd (
        ...
        .s_axi_arvalid   (plwd_ar_sel && s_axi_arvalid),
        .s_axi_rdata     (plwd_rdata),
        .s_axi_rvalid    (plwd_rvalid),
        .s_axi_awaddr    (s_axi_awaddr[3:0]),
        .s_axi_awvalid   (plwd_aw_sel && s_axi_awvalid),
        .s_axi_wdata     (s_axi_wdata),
        .s_axi_wvalid    (plwd_aw_sel && s_axi_wvalid),
    ...
    // =========================================================================
    // Pipeline Delay: align rls_error with MLP output (3-cycle latency)
    // =========================================================================
    localparam PIPE_DELAY = 3;
    reg signed [15:0] rls_err_pipe [0:PIPE_DELAY-1];
    reg               rls_vld_pipe [0:PIPE_DELAY-1];

    always @(posedge clk or negedge rst_n_sync) begin
        if (!rst_n_sync || plwd_pipeline_srst) begin
            for (i_pipe = 0; i_pipe < PIPE_DELAY; i_pipe = i_pipe + 1) begin
                rls_err_pipe[i_pipe] <= 16'sd0;
                rls_vld_pipe[i_pipe] <= 1'b0;
            end
        end else begin
            rls_err_pipe[0] <= rls_error;
            rls_vld_pipe[0] <= rls_valid_out;
            for (i_pipe = 1; i_pipe < PIPE_DELAY; i_pipe = i_pipe + 1) begin
                rls_err_pipe[i_pipe] <= rls_err_pipe[i_pipe-1];
                rls_vld_pipe[i_pipe] <= rls_vld_pipe[i_pipe-1];
            end
        end
    end
    ...
    wire signed [16:0] sub_full = {rls_err_pipe[PIPE_DELAY-1][15], rls_err_pipe[PIPE_DELAY-1]}
                                - mlp_scaled;
    ...
    lat_prof u_latprof (
        ...
        .s_axi_arvalid  (latprof_ar_sel && s_axi_arvalid),
        .s_axi_rdata    (latprof_rdata),
        .s_axi_rvalid   (latprof_rvalid)
    );

    // =========================================================================
    // AXI4-Lite Registers: leak_factor, fwes_alpha, decim_ratio
    // =========================================================================
    ...
    always @(posedge clk or negedge rst_n_sync) begin
        if (!rst_n_sync) begin
            ...
            // rlsctl_aw_sel handles 0xA0-0xAC writes
            if (rlsctl_aw_sel && s_axi_awvalid && s_axi_wvalid) begin
    ...
    // =========================================================================
    // Address[6:4] selects module:
    //   0 = AFFHC, 1 = AAD, 2 = CBPF, 3 = PLWD, 4 = RCIM
    //   5 = RLSCTL (leak_factor, fwes_alpha, decim_ratio)
    //   6 = LATPROF (read-only)
    wire [2:0] aw_sel = s_axi_awaddr[6:4];
    wire affhc_aw_sel  = (aw_sel == 3'd0);
    wire aad_aw_sel    = (aw_sel == 3'd1);
    wire cbpf_aw_sel   = (aw_sel == 3'd2);
    wire plwd_aw_sel   = (aw_sel == 3'd3);
    wire rcim_aw_sel   = (aw_sel == 3'd4);
    wire rlsctl_aw_sel = (aw_sel == 3'd5);
    wire other_aw_sel  = (aw_sel >= 3'd6);  // latprof is read-only on write

    wire [2:0] ar_sel = s_axi_araddr[6:4];
    wire plwd_ar_sel    = (ar_sel == 3'd3);
    wire latprof_ar_sel = (ar_sel == 3'd6);

    // Write-ready collection
    wire affhc_awready;
    wire affhc_wready;
    wire aad_awready;
    wire aad_wready;
    ...
    assign s_axi_awready = affhc_awready || aad_awready || cbpf_awready
                         || plwd_awready || rcim_awready || rlsctl_aw_sel || other_aw_sel;
    assign s_axi_wready  = affhc_wready  || aad_wready  || cbpf_wready
                         || plwd_wready  || rcim_wready  || rlsctl_aw_sel || other_aw_sel;
