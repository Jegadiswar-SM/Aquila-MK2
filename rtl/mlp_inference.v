...
module mlp_inference (
    ...
    always @(*) begin
        for (idx = 0; idx < 16; idx = idx + 1) begin
            if (current_layer == 3'd0) begin
                if (idx < 8)
                    act_val[idx] = feat_reg[idx];
                else
                    act_val[idx] = 16'sd0;
            end else if (current_layer == 3'd1) begin
                act_val[idx] = h1[(step_cnt << 4) + idx];
            end else if (current_layer == 3'd2) begin
                act_val[idx] = h2[(step_cnt << 4) + idx];
            end else begin
                act_val[idx] = h3[(step_cnt << 4) + idx];
            end
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            y_out         <= 16'sd0;
            valid_out     <= 1'b0;
            for (idx = 0; idx < 8; idx = idx + 1) feat_reg[idx] <= 16'sd0;
            for (idx = 0; idx < 128; idx = idx + 1) begin
                h1[idx] <= 16'sd0;
                h3[idx] <= 16'sd0;
            end
            for (idx = 0; idx < 384; idx = idx + 1) h2[idx] <= 16'sd0;
        end else begin
            valid_out <= 1'b0;
            ...
            case (state)
                STATE_IDLE: begin
                    if (en_in) begin
                        for (idx = 0; idx < 8; idx = idx + 1) begin
                            feat_reg[idx] <= feat_in_unpacked[idx];
                        end
                        current_layer <= 3'd0;
                        node_cnt      <= 9'd0;
                        step_cnt      <= 6'd0;
                ...
                STATE_READ_BIAS: begin
                    ren <= 1'b1;
                    case (current_layer)
                        3'd0: raddr <= 13'd64   + (node_cnt >> 4); // B1
                        3'd1: raddr <= 13'd3144 + (node_cnt >> 4); // B2
                        3'd2: raddr <= 13'd6240 + (node_cnt >> 4); // B3
                ...
                STATE_COMPUTE: begin
                    ...
                    case (current_layer)
                        ...
                        3'd1: begin
                            raddr <= 13'd72 + (node_cnt * 8) + step_cnt; // W2
                        ...
                        3'd2: begin
                            raddr <= 13'd3168 + (node_cnt * 24) + step_cnt; // W3
                ...
                STATE_WAIT_DATA: begin
                    ...
                    if (rvalid_dly[2]) begin
                        accumulator <= accumulator +
                                       $signed(prod[0])  + $signed(prod[1])  + $signed(prod[2])  + $signed(prod[3])  +
                                       $signed(prod[4])  + $signed(prod[5])  + $signed(prod[6])  + $signed(prod[7])  +
                                       $signed(prod[8])  + $signed(prod[9])  + $signed(prod[10]) + $signed(prod[11]) +
                                       $signed(prod[12]) + $signed(prod[13]) + $signed(prod[14]) + $signed(prod[15]);
                    end
                    ...
                    if (rvalid_dly == 3'd0) begin
                        ...
                        case (current_layer)
                            3'd0: h1[node_cnt] <= activated_val;
                            3'd1: h2[node_cnt] <= activated_val;
                            3'd2: h3[node_cnt] <= activated_val;
                            3'd3: begin
                                if (final_acc[39:30] > 10'sh0)
                                    y_out <= 16'sh7FFF;
                                else if (final_acc[39:30] < -10'sh1)
                                    y_out <= 16'sh8000;
                                else
